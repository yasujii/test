...
- `threads-autopost/threads_posts.json` に、日付つきで投稿が並んでいます。
  現在は **8/21〜8/28 の40本**が入っています（毎日5枠）。
- **8/16〜8/20 の25本は Typefully が担当**します（すでに予約済み）。二重投稿を避けるため、
  この5日分は Actions のキューには入れていません（退避先: `threads-autopost/_typefully_0816_0820.json`）。
  もし Typefully 側を消して Actions に一本化したくなったら、この退避ファイルから戻せます。
